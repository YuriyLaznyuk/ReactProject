...
function ModalCustomers(props) {
    ...
    }, [name, tel, email, nameValid, telValid, emailValid]);
    ...
    if (customObj && !customer.name) {
        setName(customObj.name);
    }
    if (customObj && !customer.tel) {
        setTel(customObj.tel);
    }
    if (customObj && !customer.email) {
        setEmail(customObj.email);
    }
