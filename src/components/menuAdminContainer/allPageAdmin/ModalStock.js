...
function ModalStock(props) {
    ...
    }, [name, quantity, sale, nameValid, quantityValid, saleValid, priceValid, price]);
    ...
    // костыль
    if (items && !stockObj.name) {
        setName(items.name);
    }
    if (items && !stockObj.cols) {
        setQuantity(items.cols);
    }
    if (items && !stockObj.stock) {
        setSale(items.stock);
    }
    if (items && !stockObj.price) {
    ...
    return (
        <div className='ModalStock'>
            ...
            <form action="" className='ModalStock_form'>
                ...
                {(!quantityValid && quantity.length > 0) &&
                <span className='ModalStock-valid_span'>Invalid Quantity</span>}
                ...
                <input type="text" id='sale' onChange={e => setSale(e.target.value)}
                       ...
                       style={{border: (!saleValid && sale.length > 0) && "2px solid red"}}/>
